<script>
 ...
 @if (session('pop_success'))
    ...
    //Form Parent submit parent-ajax-reg
    $('form#parent-ajax-reg').on('submit', function(ev){
        ev.preventDefault();
        submitParentForm($(this), 'store');
...
//        Update Values
    ...
    function submitParentForm(form, formType){
        var btn = form.find('button[type=submit]');
        disableBtn(btn);
        var ajaxOptions = {
            url:form.attr('action'),
        ...
        req.done(function(resp){
            console.log(resp);
            resp.msg.ok && resp.msg.msg
               ? flash({msg:resp.msg.msg, type:'success'})
               : flash({msg:resp.msg.msg, type:'danger'});

               $('#parentName').text(resp.msg.parent_name);
               //$('#pid').val(resp.msg.id);
               $('#my_pid').val(resp.msg.id);
               optionText = resp.msg.parent_name;
            var optionValue = resp.msg.id;
            //alert(optionValue);
  
            $('#my_parent_id').append(`<option value="${optionValue}">
                                       ${optionText}
                                  </option>`);
            hideAjaxAlert();
            enableBtn(btn);
