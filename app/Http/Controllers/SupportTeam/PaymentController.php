...
class PaymentController extends Controller
{
    ...
    public function select_year(Request $req)
    {
        ...
        $d['payments'] = $p = $this->pay->getPayment(['year' => $year, 'month' => $month])->get();
        // if(($p->count() < 1)){
        //     return Qs::goWithDanger('payments.index', $d);
        // }
    ...
    public function invoice($st_id, $month = NULL,  $year = NULL)
    {
        if(!$st_id) {return Qs::goWithDanger();}

        $inv = $year ? $this->pay->getAllMyPR($st_id, $month) : $this->pay->getAllMyPR($st_id);
    ...
    public function manage($month=NULL, $class_id = NULL)
    {
        $d['my_classes'] = $this->my_class->all();
        $d['selected'] = false;
        $d['month'] = $month ? $month : intval(date('m'));
    ...
    public function select_class(Request $req)
    {
        ...
        $pay1 = $this->pay->getPayment(['my_class_id' => $class_id, 'year' => $this->year, 'month'=>$month])->get();
        $pay2 = $this->pay->getGeneralPayment(['year' => $this->year, 'month'=>$month])->get();
        ...
        return redirect('/payments/manage/'  . $month . '/'  . $class_id);
    }
