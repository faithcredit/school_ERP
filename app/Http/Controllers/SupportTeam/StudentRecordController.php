...
class StudentRecordController extends Controller
{
   ...
   public function __construct(LocationRepo $loc, MyClassRepo $my_class, UserRepo $user, StudentRepo $student)
   {
    ...
    public function create()
    {
        ...
        $data['cities'] = $this->loc->getLGAs(4);
        return view('pages.support_team.students.add', $data);
    }
    public function create_o() {
        $data['my_classes'] = $this->my_class->all();
        $data['parents'] = $this->user->getUserByType('parent');
        $data['dorms'] = $this->student->getAllDorms();
        $data['states'] = $this->loc->getStates();
        $data['nationals'] = $this->loc->getAllNationals();
        $data['cities'] = $this->loc->getLGAs(4);
        return view('pages.support_team.students.add_o', $data);
    }
    ...
    public function store(StudentRecordCreate $req)
    {
       ...
       /* $ct = ($ct == 'J') ? 'JSS' : $ct;
        ...
        $users = $this->user->findByEmail($req->email);
        if($users && $users->count() > 0) {
            return Qs::json('The same email already exists.', FALSE);
        }
    ...
    public function storeParent(Request $req)
    {
        ...
        $uname = strtolower(preg_replace('/\s+/', '', $req->name));

       $data['username'] = $uname;

        $pass = $uname ?: $user_type;
        $data['password'] = Hash::make($pass);

   

       $last_inserid = DB::table('users')->insertGetId(array(
        'name'  =>  $data['name'],
        'code'  =>  $data['code'],
